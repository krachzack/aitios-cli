...
use runner;
use files::{fs_timestamp, create_file_recursively};
...
fn init_logging(matches: &ArgMatches, additional_log_path: &Option<PathBuf>, datetime: &str) -> Result<(), Error> {
    configure_logging(matches, additional_log_path.as_ref().map(|p| p.to_string_lossy()).iter(), datetime)
...
fn configure_logging<I, S>(arg_matches: &ArgMatches, additional_logs: I, datetime: &str) -> Result<(), Error>
        ...
        S : AsRef<str>
{
    ...
    for log in log_paths.into_iter() {
        let log = create_file_recursively(log)
            .context("Failed to create log file.")?;

        loggers.push(
            WriteLogger::new(LevelFilter::Debug, Config::default(), log)
        );
    }
...
fn canonical_log_file_paths<I, S>(arg_matches: &ArgMatches, additional_logs: I, datetime: &str) -> Result<HashSet<PathBuf>, Error>
    where I : IntoIterator<Item = S>,
        S : AsRef<str>
{
    ...
    if let Some(log_file_args) = arg_matches.values_of("log") {
        log_files.extend(
            log_file_args.map(|a| log_arg_to_log_path(a, datetime))
        )  
    }
...
mod test {
    ...
    fn test_log_arg_with_datetime() {
        ...
        let len_until_day_of_month = current_dir().unwrap().to_str().unwrap().len() + "/logovic-2018-07-17T18_06_53".len();
        assert_eq!(
            expected.to_str().unwrap().to_string().truncate(len_until_day_of_month),
            actual.to_str().unwrap().to_string().truncate(len_until_day_of_month)
        )
    }
    ...
    fn test_default_log_name_added() {
        ...
        let log_file_paths = canonical_log_file_paths(
            ...
            iter::empty::<&str>(),
            &fs_timestamp(Local::now())
        ).expect("Expect canonical log file calculation to succeed with value-less log switch");
        ...
        // datetime formattings
        let time = Local::now();
        let len_until_day_of_month = current_dir().unwrap().to_str().unwrap().len() + "/".len() + synthesize_datetime_log_filename(&fs_timestamp(time)).len() - "_53.800963444+02_00.log".len();
        let mut expected = {
            let mut expected = current_dir().unwrap();
            expected.push(synthesize_datetime_log_filename(&fs_timestamp(time)));
            expected.to_str().unwrap().to_string()
        };
        let mut actual = log_file_paths.into_iter().next().unwrap().to_str().unwrap().to_string();
    ...
    fn directory_as_log_path() {
        ...
        assert_eq!(
            1,
            log_file_paths.len(),
            "Expected 1 log file under the parent directory but got {:?}.", &log_file_paths
        );
        ...
        assert_eq!(
            parent_dir,
            parent_of_log_file_path,
            "Expected generated default log file to be in parent directory but got {:?}.", &log_file_paths
        );
    }
