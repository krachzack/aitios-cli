...
pub enum SurfelRuleSpec {
    Transfer { from: String, to: String, factor: f32 },
    Deteriorate { from: String, factor: f32 }
}
...
mod test {
    ...
    fn test_parse_surfel_spec() {
        ...
        let mut surfel_spec_file = File::open(path)
            .expect("Test surfel spec could not be opened");
